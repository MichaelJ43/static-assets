...
- `src/m43-auth-header.ts` / `src/auth-header-core.ts` — optional **top bar** (logo, Home, **Log In** / profile menu) for **shared-api-platform** (`GET /v1/auth/me`, `POST /v1/auth/logout`, `returnUrl` via auth origin). Bundled to `dist/v1/m43-auth-header.js`.
